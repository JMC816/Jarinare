...
const PCTicketReturnModal = () => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[60] hidden items-center justify-center bg-black/40 lg:flex">
      <div className="animate-fade-up w-[420px] rounded-2xl bg-white shadow-xl">
        {/* 대제목 + X버튼 */}
        ...
        <div className="flex flex-col gap-3 px-6 pb-6">
          ...
          <div className="rounded-xl border border-gray-100 bg-gray-50 px-5 py-4">
            ...
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-400">{seat.startDayForView}</span>
            </div>
            ...
            <div className="mb-3 flex items-center gap-3">
              <div className="flex flex-col items-center gap-0.5">
                ...
                <span className="text-xs text-gray-500">{seat.startStationForView}</span>
              </div>
              <div className="flex flex-1 items-center">
                <div className="h-1.5 w-1.5 shrink-0 rounded-full bg-blue" />
                <div className="flex-1 border-t border-blue/30" />
                <span className="mx-2 shrink-0 text-[10px] text-gray-400">{duration}</span>
                <div className="flex-1 border-t border-blue/30" />
                <div className="h-1.5 w-1.5 shrink-0 rounded-full bg-blue" />
              </div>
              <div className="flex flex-col items-center gap-0.5">
                <span className="text-lg font-bold text-gray-900">{endTime}</span>
                <span className="text-xs text-gray-500">{seat.endStationForView}</span>
              </div>
            </div>
            ...
            {/* 인원수 */}
            <p className="text-center text-sm font-semibold text-gray-700">{passengerText}</p>
          </div>

          {/* 안내 메시지 카드 */}
          <div className="flex items-center gap-2 rounded-xl px-4 py-3" style={{ backgroundColor: 'rgba(234, 67, 53, 0.08)' }}>
            <svg
              width="16"
              height="16"
              viewBox="0 0 24 24"
            ...
            <span className="text-xs font-bold text-red">반환 후에는 되돌릴 수 없어요.</span>
          </div>
