/**
 * @role: features/api — 이벤트 좋아요 조회·구독·토글
...
export const fetchEventLikedStateApi = async (
  uid: string,
): Promise<Record<string, boolean>> => {
  const snap = await getDocs(collection(db, 'isLiked', uid, 'eventVotes'));
...
export const subscribeEventLikeCountApi = (
  docId: string,
...
export const toggleEventLikeApi = async (
  ...
  const likeRef = doc(db, 'isLiked', uid, 'eventVotes', docId);
  const countRef = doc(db, 'eventLikes', docId);
