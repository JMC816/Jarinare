/**
 * @role: widgets — PC 출발역 드롭다운 UI
 * @rule: 렌더링만 담당, onClose prop으로 닫기 처리
 */
import { usePCStartPlaceDropdown } from '../hooks/usePCStartPlaceDropdown';
import type { OnCloseProps } from '../types/PCDropdownTypes';

const PCStartPlaceDropdown = ({ onClose }: OnCloseProps) => {
  const {
    query,
    setQuery,
    filtered,
    ...
    handleSelect,
  } = usePCStartPlaceDropdown(onClose);
...
export default PCStartPlaceDropdown;
