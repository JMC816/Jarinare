/**
 * @role: features/api — 자유게시판 좋아요 조회·구독·토글
...
export const fetchBoardLikedStateApi = async (
  uid: string,
): Promise<Record<string, boolean>> => {
  const snap = await getDocs(collection(db, 'isLiked', uid, 'boardVotes'));
...
export const subscribeBoardLikeCountApi = (
  docId: string,
...
export const toggleBoardLikeApi = async (
  ...
  const likeRef = doc(db, 'isLiked', uid, 'boardVotes', docId);
  const countRef = doc(db, 'boardLikes', docId);
