...
지방 거주로 인해 기차를 자주 이용하는 사용자들의 경험을 개선하고, 연인이나 가족 단위로 좌석을 따로 예약해야 하는 불편함을 해결하고자 **자리나래**를 개발하게 되었습니다.
자리나래는 실제 기차 운행 스케줄을 연동하여 **좌석 조회, 선택, 변경, 결제까지 통합 지원**하는 기차 예매 플랫폼입니다.
사용자는 단일 또는 다중 좌석을 예약할 수 있으며, **실시간 좌석 잠금/해제 기능**으로 동시 사용자의 중복 선택을 방지합니다. 또한 좌석 변경 요청, 승인/거부 알림, 포인트 적립 등 **상호작용 기반의 편리한 예약 경험**을 제공합니다.
